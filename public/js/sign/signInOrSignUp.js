	...
	if($('#nombre').length){//Significa que estamos en Sign Up
		...
		if(validarSignUp(emailDeRegistro, passwordDeRegistro, nombreDeRegistro, telefonoDeRegistro)){
			firebase.auth().createUserWithEmailAndPassword(emailDeRegistro, passwordDeRegistro).then(function(user){
				...
				firebase.database().ref('choferes/' + user.uid).set({nombre: nombreDeRegistro, email: emailDeRegistro, telefono: telefonoDeRegistro}, function(error){
					if(error){
						...
						if(foto[0].files[0] != undefined){//Si seleccionaron foto, hacemos la subida
							...
							uploadTask.on('state_changed', function(snapshot){
								...
								firebase.database().ref().child('choferes/' + user.uid).update({
		...
		if(validarSignIn(email, password)){
			firebase.auth().signInWithEmailAndPassword(email, password).then(function(user){
				...
				firebase.database().ref('choferes/' + user.uid).once('value', function(snapshot){
					if(snapshot.val() != null){
						...
						alert('Un perfil de usuario no puede utilizar la aplicación como conductor');//TODO: Quitamos alert por notificación
					}
				});
			}).catch(function(error) {
				var errorCode = error.code;
...
function generaCodigoAleatorio(usuario){
	...
	firebase.database().ref('choferes/' + usuario + '/codigoUsuarioActual').set(code);
